...
import { getTopRatedTvShows } from '@/lib/tmdb/tv-shows'
import { Fragment } from 'react'

type SearchParams = Promise<{ [key: string]: string | string[] | undefined }>

export default async function TopRatedTvShowsPage(props: { searchParams: SearchParams }) {
  const searchParams = await props.searchParams
  const page = Number(searchParams.page || 1)

  const [error, tvShows] = await getTopRatedTvShows(page)
