...
const BackButton = ({ variant, asLink, href, ...props }: ButtonProps & ({ asLink?: false; href?: string } | { asLink: true; href: string })) => {
  const children = (
    <>
      <Undo2Icon size={16} /> <p>Back</p>
    </>
  )

  if (asLink)
    return (
      <Button {...props} variant={variant || 'link'} asChild>
        <Link href={href}>{children}</Link>
      </Button>
    )

  return (
    <Button {...props} variant={variant || 'link'}>
      {children}
    </Button>
  )
}

export { BackButton, Button, buttonVariants }
