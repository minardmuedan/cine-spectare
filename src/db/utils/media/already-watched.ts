...
export const getUserAlreadyWatchedDb = async (userId: string) =>
  await db.select({ id: alreadyWatchTable.id, media: alreadyWatchTable.media }).from(alreadyWatchTable).where(eq(alreadyWatchTable.addedBy, userId))
...
export const getUserAlreadyWatchedIdDb = async (userId: string) =>
  await db
    .select({
